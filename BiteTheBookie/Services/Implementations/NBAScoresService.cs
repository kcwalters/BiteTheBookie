    ...
    public class NBAScoresService : INBAScoresService
    {
        ...
        public async Task<IReadOnlyList<NBATickerView>> GetGamesAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                if (events.Count == 0)
                {
                    _logger.LogInformation("ESPN NBA scoreboard: no games today or upcoming.");
        ...
        public async Task<IReadOnlyList<NBATickerView>> GetGamesForDateAsync(DateTime date, CancellationToken cancellationToken = default)
        {
            try
            {
            ...
            {
                _logger.LogError(ex, "Error fetching NBA scoreboard for date {Date}", date);
                return Array.Empty<NBATickerView>();
            }
        }
